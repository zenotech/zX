...
- Ensure a secure connection to the backend by using a random token that is generated by the client and used for api access on the backend. The token should be stored in the backend and used for all api calls.
...
## Packaging
1. The application should be packaged as a standalone installable package for macOS, Windows, and Linux.

## Development Environment
1. Generate scripts to enable live development using vite.